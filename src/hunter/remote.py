...
class RemoteStream(object):
    ...
    def write(self, data):
        try:
            ...
            print(
                "Hunter failed to send trace output {!r} (encoding: {!r}): {!r}. Stopping tracer.".format(
                    data, self._encoding, exc
                ),
                file=sys.stderr,
            )
            stop()
...
def gdb_bootstrap(args, activation_payload, deactivation_payload):
    ...
    activation_command = [
        ...
        "-batch",
        "-ex",
        "call (void)Py_AddPendingCall(PyRun_SimpleString, {})".format(
            json.dumps(activation_payload)
        ),
    ]
    deactivation_command = [
        ...
        "-ex",
        "call (void)Py_AddPendingCall(PyRun_SimpleString, {})".format(
            json.dumps(deactivation_payload)
        ),
    ]
    ...
    try:
        ...
        stream.write(
            "Failed to activate: {!r}. {}\n".format(
                exc,
                "Tracer options where: {}.".format(options)
                if options
                else "No tracer options.",
            )
        )
...
def main():
    ...
    with bootstrapper(args, payload, "from hunter import remote; remote.deactivate()"):
        ...
        if pid:
            if pid != args.pid:
                raise Exception(
                    "Unexpected pid {!r} connected to output socket. Was expecting {!r}.".format(
                        pid, args.pid
                    )
                )
        else:
